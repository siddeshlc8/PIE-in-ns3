...
int main (int argc, char *argv[])
{
  ...
  Config::SetDefault ("ns3::PieQueueDisc::MaxSize", StringValue ("500p"));
  ...
  // uint16_t port = 50000;
  // InstallPacketSink (sink.Get (0), port);      // A Sink 0 Applications

  // InstallBulkSend (source.Get (0), interfaces [0].GetAddress (1), port);
  // InstallBulkSend (source.Get (1), interfaces [1].GetAddress (1), port);
  // InstallBulkSend (source.Get (2), interfaces [2].GetAddress (1), port);
  // InstallBulkSend (source.Get (3), interfaces [3].GetAddress (1), port);
  // InstallBulkSend (source.Get (4), interfaces [4].GetAddress (1), port);
  ...
  if (writeForPlot)
    {
      ...
      filePlotQueueDelay << pathOut << "pie-queue-delay"+ext+".plotme";
      filePlotQueueSize << pathOut  << "pie-queue-size"+ext+".plotme";
